...
# AIW_PATCH_WRAP_JSON_DUMP_V1: ensure risk_bucket/expected_return_pct/downside_pct/rr_ratio exist in API JSON
def _aiw__rb_from_profile(prof: str) -> str:
    u = (prof or "").upper()
...
def _aiw__f(x, default=0.0):
    try: return float(x)
    except Exception: return float(default)

def _aiw_postprocess(obj):
    try:
        if isinstance(obj, dict):
            trades = obj.get("proposed_trades") or []
            for t in trades:
                ...
                if t.get("risk_bucket") in (None, "", "None"):
                    t["risk_bucket"] = _aiw__rb_from_profile(prof)

                entry  = _aiw__f(t.get("entry_price"))
                target = _aiw__f(t.get("target_price"))
                sl     = _aiw__f(t.get("stop_loss"))
                ...
                if t.get("expected_return_pct") in (None, "", "None"):
                    t["expected_return_pct"] = round(exp, 2)
                if t.get("downside_pct") in (None, "", "None"):
                    t["downside_pct"] = round(down, 2)
                if t.get("rr_ratio") in (None, "", "None"):
                    t["rr_ratio"] = round(rr, 2)
    except Exception:
        pass
    # Back-compat alias keys for UI (keep UI frozen)
    try:
        for t in (obj.get("proposed_trades") or []):
            if t.get("stop_loss_price") in (None, "", "None"):
                t["stop_loss_price"] = t.get("stop_loss")
            if t.get("downside_risk_pct") in (None, "", "None"):
                t["downside_risk_pct"] = t.get("downside_pct")
            if t.get("confidence_score") in (None, "", "None"):
                t["confidence_score"] = t.get("confidence")
    except Exception:
        pass
...
def main():
    ...
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        ...
        json.dump(_aiw_postprocess(payload), f, indent=2)
