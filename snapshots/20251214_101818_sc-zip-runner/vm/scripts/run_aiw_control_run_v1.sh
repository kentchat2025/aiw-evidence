...
sudo env OUTPUT_PATH="/opt/founderconsole/runtime/aiw_validation_report.json" python3 /opt/founderconsole/scripts/aiw_build_validation_report_v1.py --date "${RUN_DATE}"

sudo docker cp /opt/founderconsole/runtime/aiw_validation_report.json founderconsole-backend:/opt/founderconsole/runtime/aiw_validation_report.json
...
# --- AIW-EVIDENCE:EXPORT (post control-run; non-blocking) ---------------------
if [ -x /opt/founderconsole/scripts/aiw_evidence_export_after_run.sh ]; then
  echo "[AIW-EVIDENCE] Exporting evidence snapshot to GitHub (post-run)..." | tee -a /opt/founderconsole/runtime/aiw_evidence_export.log
  /opt/founderconsole/scripts/aiw_evidence_export_after_run.sh >> /opt/founderconsole/runtime/aiw_evidence_export.log 2>&1 || true
else
  echo "[AIW-EVIDENCE] Wrapper missing: /opt/founderconsole/scripts/aiw_evidence_export_after_run.sh" | tee -a /opt/founderconsole/runtime/aiw_evidence_export.log
fi
# --- AIW-EVIDENCE:EXPORT ------------------------------------------------------
