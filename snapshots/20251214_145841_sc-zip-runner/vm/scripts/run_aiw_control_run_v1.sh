...
echo "[AIW-CONTROL] Expanding creamy layer to all profiles (if missing) for ENV=${ENV_CODE}, RUN_DATE=${RUN_DATE}"
python3 /opt/founderconsole/scripts/aiw_expand_creamy_to_all_profiles_v1.py --env "${ENV_CODE}" --date "${RUN_DATE}"

echo "[AIW-CONTROL] Enriching creamy layer metrics (profile-driven) for ENV=${ENV_CODE}, RUN_DATE=${RUN_DATE}"
sudo /usr/bin/env python3 /opt/founderconsole/scripts/aiw_profile_metrics_enricher_v2.py --db "${DB_PATH}" --date "${RUN_DATE}" --env "${ENV_CODE}"
